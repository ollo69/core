...
async def test_connect_fail_http(
    ...
    connect_http.return_value.async_connect = AsyncMock(side_effect=side_effect)
    connect_http.return_value.async_get_settings = AsyncMock()
...
async def test_unique_id_migration(hass: HomeAssistant, connect_http) -> None:
